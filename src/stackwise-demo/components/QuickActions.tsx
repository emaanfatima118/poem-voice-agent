...
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@/stackwise-demo/components/ui/tooltip'
...
export function QuickActions({ module = 'BrandCraft', onNewInsight, inline = false }: QuickActionsProps) {
  ...
  const simulateResponse = (msg: string, cb: () => void) => {
    setLoading(true)
  ...
  const handleAccept = (item: string) => {
    setOutput((prev: any) => ({
      ...prev,
      results: prev.results.map((r: string) => (r === item ? `✔️ Accepted: ${item}` : r)),
    }))
  }
        ...
        {output && (
          ...
          >
            ...
            <ul className="space-y-2">
              {output.results.map((item: string, i: number) => (
