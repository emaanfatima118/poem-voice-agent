...
// Type definitions (since @shared/schema doesn't exist in this project)
type Project = {
  id: string;
  userId: string;
  name: string;
...
type StepKey = 'dashboard' | 'voice-check' | 'compare' | 'insights';
...
<head>
  ...
  <title>Brand Voice Enforcement Report</title>
  <style>
...
<body>
  <h1>Brand Voice Enforcement Report</h1>
  ...
  <div class="footer">
    Brand Voice Enforcement Dashboard &middot; ${new Date().getFullYear()}
  </div>
</body>
</html>
    `;
  ...
  const steps = [
    ...
    { id: 'voice-check', label: 'Voice Check', description: 'Analyze assets for voice consistency' },
    { id: 'compare', label: 'Compare Assets', description: 'Side-by-side asset comparison' },
  ...
  const leftNav = (
    <div className="p-6">
      ...
      <p className="text-sm text-muted-foreground mt-1">Brand Voice Enforcement</p>
    </div>
  );
      ...
      <div className="h-full overflow-y-auto bg-white">
        <div className="bg-white px-8 py-4 border-b border-pink-100 sticky top-0 z-10">
          <div className="flex justify-between items-center flex-wrap gap-3">
            <div>
              <h1 className="text-3xl font-bold mb-2" style={{ color: moduleColor }}>Brand Voice Enforcement</h1>
              <p className="text-gray-600 max-w-3xl">
                Ensure brand consistency across all content and campaigns with AI-powered voice analysis.
              </p>
            </div>
            <div className="flex gap-2">
        ...
        {currentStep === 'voice-check' && renderVoiceCheck()}
        {currentStep === 'compare' && renderCompare()}
